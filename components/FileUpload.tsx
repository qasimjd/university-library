  ...
  const onValidate = (file: File) => {
    if (type === "image") {
      if (file.size > 20 * 1024 * 1024) {
        toast.error("File size too large, Please upload a file that is less than 20MB in size");

        return false;
      }
    } else if (type === "video") {
      if (file.size > 50 * 1024 * 1024) {
        toast.error("File size too large, Please upload a file that is less than 50MB in size");
        return false;
      }
    }
